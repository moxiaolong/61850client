...
func (p *ConfirmedRequestPDU) decode(is *bytes.Buffer, withTag bool) int {
	tlByteCount := 0
	vByteCount := 0
	numDecodedBytes := 0
...
func (p *ConfirmedRequestPDU) encode(reverseOS *ReverseByteArrayOutputStream, withTag bool) int {
	if p.code != nil {
...
func NewConfirmedRequestPDU() *ConfirmedRequestPDU {
	return &ConfirmedRequestPDU{tag: NewBerTag(0, 32, 16)}
}
